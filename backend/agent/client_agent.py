    ...
    try:
        ...
        firestore_payload = {
            "fields": {
                ...
                "cpu_usage": {"doubleValue": float(payload.get("cpu_usage") or 25.0)},
                "ram_usage": {"doubleValue": float(payload.get("ram_usage") or 55.0)},
                "disk_usage": {"doubleValue": float(payload.get("disk_usage") or 45.0)},
                "battery_percent": {"doubleValue": float(payload.get("battery_percent") or 90.0)},
                "temperature_current": {"doubleValue": float(payload.get("temperature_current") or 45.0)},
                "uptime_hours": {"doubleValue": float(payload.get("uptime_hours") or 10.0)},
                "shutdowns_30d": {"integerValue": int(payload.get("shutdowns_30d") or 0)},
